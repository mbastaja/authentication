const SuperEditService = require('../services/superEditService')
...
const CheckSuperService = require('../services/checkSuperService')


exports.superEdit = async (req, res, next) => {
    CheckSuperService(req.decoded.id, function (err, result) {
        if (err) throw err
        if (result) {
            let currentUsername = '';
            let currentEmail = '';
                ...
                if (err) {
                    ...
                    CheckEmailService(req.body.email, function (err, result) {
                        ...
                        if (err) {
                            ...
                            SuperEditService(req, function (err, user) {
                                if (err) throw err
                                if (user) {
                                    res.status(201).json({
